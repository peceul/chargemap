let url = "https://open.jejudatahub.net/api/proxy/atDab6t8218btaa122b26DDtbatD86t1/08_jj03ro_b8c08tjr1cejt8c8c1bbjt";
    ...
    for (let i = 0; i < datas.length; i++) {
        ...
        if (info != null) { // Null 발생시 통과            
            ...
            if (quickChargingType == "") {
                ...
                iwContent = '<div style="margin : 10px; width:200px; height:225px;">';
            }
            iwContent += '<div style="margin : 10px; width:200px; height:200px;">' +
            ...
            iwContent += '<div style="font-weight:bold; font-size:14px;">완속 충전 여부</div>' +
                         ...
                         //'<div><input type="button" value="주변시설 보기" onclick="find(' + i + ');"></div>' +
                         '</div>';
...
/*function find(seq) {
    //chargingPlace, addressJibun, quickChargingType, chargingFlag, quickChargingFlag, 
    let reg = confirm("이 충전소 주변의 위치 정보를 확인하시겠습니까?");
    if (reg) {
        ...
        window.location.assign(window.location.href.split('location.html')[0] + "recommend.html" +
            "?latitude=" + datas[seq]["latitude"] +
            "&longitude=" + datas[seq]["longitude"] +
            "&chargingPlace=" + datas[seq]["chargingPlace"] +
            "&addressJibun=" + datas[seq]["addressJibun"] +
            "&quickChargingType=" + datas[seq]["quickChargingType"] + "" +
            "&chargingFlag=" + (datas[seq]['chargingFlag'] == true ? "가능" : datas[seq]['chargingFlag'] == false ? "불가능" : "") +
            "&quickChargingFlag=" + (datas[seq]['quickChargingFlag'] == true ? "가능" : datas[seq]['quickChargingFlag'] == false ? "불가능" : "")
        );
    } else {
        return false;
    }
